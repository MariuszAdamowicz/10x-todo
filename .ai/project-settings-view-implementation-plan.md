...
- ProjectSettingsPage.astro (Wrapper strony, pobiera dane serwerowo)
  - ProjectSettingsView.tsx (Główny komponent kliencki, zarządza stanem)
    - ui/Button (przycisk "Powrót do projektu")
...
- **Główne elementy**: Przycisk nawigacyjny, `ProjectSettingsForm`, `ApiKeyManager`, `DangerZone`.
- **Obsługiwane interakcje**:
  - Kliknięcie przycisku "Powrót do projektu" nawiguje użytkownika do `/projects/[id]`.
  - Pozostałą logikę deleguje do komponentów podrzędnych.
...
- **Obsługiwane interakcje**:
  - Wartość pola `Input` powinna być powiązana ze stanem. Jeśli `isApiKeyVisible` jest `true`, wyświetla `project.apiKey`; w przeciwnym razie wyświetla zamaskowaną wartość (np. `••••••••••••••••`).
  - Kliknięcie przycisku "Pokaż/Ukryj" zmienia widoczność klucza.
  - Kliknięcie przycisku "Kopiuj" kopiuje `project.apiKey` do schowka.
...
- **Stan**:
  - `project: ProjectSettingsViewModel` - główny obiekt stanu widoku, **zainicjalizowany z `initialProject`**.
...
- **Funkcje**:
  ...
  - `copyApiKey()`: Kopiuje `project.apiKey` do schowka i wyświetla powiadomienie.
  - `regenerateApiKey()`: Wywołuje `POST /api/projects/[id]/regenerate-api-key`. Po sukcesie **aktualizuje stan `project.apiKey` nowym kluczem** zwróconym przez API.
...
- **Nawigacja**: Użytkownik klika "Powrót do projektu" i zostaje przeniesiony na stronę `/projects/[id]`.
...
- **Regeneracja klucza**: Użytkownik klika "Generuj nowy klucz", otwiera się modal. Po potwierdzeniu, akcja jest wykonywana. W przypadku sukcesu modal jest zamykany, a **nowy klucz pojawia się w polu**, ponieważ stan `project.apiKey` został zaktualizowany.
...
2.  **Główny komponent React**: Zaktualizowanie `ProjectSettingsView.tsx` o przycisk "Powrót do projektu" i upewnienie się, że przyjmuje dane z Astro (`client:load`).
3.  **Implementacja hooka `useProjectSettings`**: Zdefiniowanie stanu i logiki dla wszystkich operacji (CRUD), upewniając się, że `apiKey` jest poprawnie inicjowany i aktualizowany.
...
5.  **Komponent `ApiKeyManager`**: Implementacja logiki wyświetlania, maskowania, kopiowania i regeneracji klucza API. Upewnienie się, że pole `Input` poprawnie reaguje na zmianę `isApiKeyVisible`. Użycie `AlertDialog` z `shadcn/ui` do potwierdzenia.

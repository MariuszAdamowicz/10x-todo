...
2. Analyze the error. If it's a wrong ID, verify with 
get_task_hierarchy
.
3. If unrecoverable, call 
propose_task_resolution(parentId, 'cancelled', error_message)
...
You must always use 
reorder_tasks
 to maintain this exact order (Highest to Lowest):
...
- **NO (Complex):**
    1. Call 
create_subtask
 multiple times to break it down.
    2. Call 
reorder_tasks
 to sort them by Priority Rules.
...
- **MCP:** Call 
create_subtask(parentId, "RED: Write test for [Task]")
.
- **MCP:** Call 
reorder_tasks
 (Put RED at the top).
- **Status:** You may mark the Generic Task as 
`in_progress`
.
- **STOP.**
...
- **MCP:** Call 
create_subtask(parentId, "GREEN: Implement [Task] to pass test")
.
- **MCP:** Call 
update_subtask_status(redTaskId, 'done')
.
- **MCP:** Call 
reorder_tasks
 (Put GREEN at the top).
- **STOP.**
...
- **MCP:** Call 
create_subtask(parentId, "REFACTOR: Optimize [Task]")
.
- **MCP:** Call 
update_subtask_status(greenTaskId, 'done')
.
- **MCP:** Call 
reorder_tasks
 (Put REFACTOR at the top).
- **STOP.**
...
- **Verify:** Run test -> Confirm PASS.
- **MCP:** Call 
update_subtask_status(refactorTaskId, 'done')
.
- **MCP:** Call 
reorder_tasks
 (Check if parent task is fully complete).
- **STOP.**

#### STEP 4: COMPLETION
- If the Main Delegated Task has all subtasks marked 'done':
- Call 
propose_task_resolution(delegatedTaskId, 'done', summary)
.
...
- **Reorder Always:** Never skip 
reorder_tasks
. The list must always reflect the Priority Rules.
`
        },
      },
    ],

...
export async function POST({ params, request, locals }: APIContext): Promise<Response> {
  ...
  try {
    ...
    const updatedTask = await taskService.proposeTaskStatus(taskId, command, { aiProjectId });
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("delegated tasks") || error.message.includes("Invalid status transition")) {

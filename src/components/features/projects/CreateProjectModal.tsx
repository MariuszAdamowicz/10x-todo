...
const CreateProjectModal: React.FC<CreateProjectModalProps> = ({ isOpen, onOpenChange, onSubmit }) => {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>Wypełnij poniższe pola, aby stworzyć nowy projekt.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <ProjectForm onSubmit={handleSubmit} isSubmitting={isSubmitting} />
        </div>
        <DialogFooter>{/* Można tu dodać dodatkowe przyciski, np. "Anuluj" */}</DialogFooter>
      </DialogContent>
    </Dialog>
  );
};
